...
yorigin, xorigin = rp.svm_read_problem('mushroom_10')
...
start = time.time() 
# mushroom rate=0.000001
# queryNum = 16 32 64 128 326
rates = [0.000001]
nums = [16, 32, 64, 128, 326]
result = []
for rate in rates:
    for num in nums:
        ...
        err2 = testcluster_query(rate=rate, query_func=query.queryRand, ncluster=n_cluster,
                                 queryNum=num)
        temp = pd.DataFrame([rate, num, err1, err2], index=['rate', 'num', 'not-rand', 'rand']).T
        result.append(temp)
pd.concat(result).to_csv('mushroom'+'.csv',header = True)